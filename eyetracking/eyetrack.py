...
def preprocessing(num):
    ...
    with open('/Users/jeonsumin/PycharmProjects/eyetracking/eyetraking_from-image/new_user_credentials.csv', 'r') as input:
        next(input)
    ...
    # get image file
    # 이미지 셋 선택
    testfolder = '/Users/jeonsumin/Desktop/allonsy-git/Allons-y-back/eyetracking/testfolder/'
    if (num == 1):
        # blinking1
        select_directory_name = directory_name[num-1]
        for i in range(0, len(photo_list)):
            down = s3.download_file(bucket, "photo(youtube)/" + select_directory_name + "/" + photo_list[i],
        ...
        # shaking - JSM
        ImgArray = np.array(
            ["/Users/jeonsumin/PycharmProjects/eyetracking/eyetraking_from-image/photo(jeonSM)/shaking1/1.jpg",
             "/Users/jeonsumin/PycharmProjects/eyetracking/eyetraking_from-image/photo(jeonSM)/shaking1/2.jpg",
             "/Users/jeonsumin/PycharmProjects/eyetracking/eyetraking_from-image/photo(jeonSM)/shaking1/3.jpg"])
    elif (num == 10):
        # man1 -
        select_directory_name = directory_name[num - 1]
        for i in range(0, len(photo_list)):
...
def process(eye_points_L, eye_points_R, facial_landmarks, _gray, frame, i):
    ...
    eye_region_L = np.array([(facial_landmarks.part(eye_points_L[0]).x, facial_landmarks.part(eye_points_L[0]).y),
                           ...
                           (facial_landmarks.part(eye_points_L[5]).x, facial_landmarks.part(eye_points_L[5]).y)], np.int32)
    ...
    eye_region_R = np.array([(facial_landmarks.part(eye_points_R[0]).x, facial_landmarks.part(eye_points_R[0]).y),
                           ...
                           (facial_landmarks.part(eye_points_R[5]).x, facial_landmarks.part(eye_points_R[5]).y)], np.int32)
...
def main():
    ...
    predictor = dlib.shape_predictor("/Users/jeonsumin/PycharmProjects/eyetracking/eyetraking_from-image/shape_predictor_68_face_landmarks.dat")
...
def afterprocessing():
    ...
    select_directory_name = directory_name[num - 1]
    for i in range(0, len(photo_list)):
        down = s3.delete_object(Bucket = bucket, Key = "photo(youtube)/" + select_directory_name + "/" + photo_list[i])

    # 로컬 저장소 삭제
    [os.remove(f)
     for f in
     glob.glob('/Users/jeonsumin/Desktop/allonsy-git/Allons-y-back/eyetracking/testfolder/*.png')]
